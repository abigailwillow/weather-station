...
public class GenerateImage(ILoggerFactory loggerFactory, IBlobProviderService blobProviderService)
{
    ...
    public async Task Run([QueueTrigger("image-generation-queue")] QueueMessage message, FunctionContext context) {
        ...
        _logger.LogInformation("Started job {JobId} for station {StationName}.", imageGenerationRequest.JobId, imageGenerationRequest.StationName); 

        _logger.LogInformation("Station: {StationName}, Temperature: {Temperature}°C", imageGenerationRequest.StationName, imageGenerationRequest.Temperature);
        ...
        image = ImageUtility.AddTextToImage(image, ($"{imageGenerationRequest.Temperature}°C", (16, 48), 32, "#FFFFFF"));
        
        using MemoryStream memoryStream = new (image);
        
        BlobClient imageBlobClient = _blob.GetBlobClient($"{imageGenerationRequest.JobId}-{imageGenerationRequest.MeasurementIndex}.png");
